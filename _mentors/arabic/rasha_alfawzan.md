---
ref: rasha_alfawzan
lang: ar
categories: ar
name: رشا الفوزان
jobtitle: مهندسة تحليلات
workplace: جينيرال الكتريك
shortbio: >-
  متخصصه في تقنية المعلومات و الروبوتات (RPA) وإدارة مشاريع تقنية المعلومات
  وتحليلات البيانات.
longbio: >-
  لدي 12 عامًا من الخبرة في مجال تقنية المعلومات. تخرجت من جامعة الملك سعود
  بدرجة البكالوريوس في علوم المعلومات. لدي أيضا درجة الماجستير من جامعة الأمير
  سلطان في إدارة الأعمال. متخصصة في إدارة مشاريع تقنية المعلومات ، والتشغيل
  الآلي لعملية الروبوتات ، وتحليلات البيانات.
published: true
image: >-
  https://raw.githubusercontent.com/not-dalia/qudwa/gh-pages/media/15515987352781551598733409_logo.jpg
tags:
  - تقنية
  - هندسة
topics:
  - التقديم للوظائف و الجامعات
  - ادارة مشاريع
  - تحليل البيانات و العمليات
interests:
  - الرياضة
  - القراءة
education-experience:
  - title: شهادة الماجستير
    place: جامعة الأمير سلطان
    description: إدارة الأعمال
    duration: ٢٠١٥
  - title: شهادة البكالوريس
    place: جامعة الملك سعود
    description: تقنية المعلومات
    duration: ٢٠٠٦
social-accounts:
  linkedin: 'https://www.linkedin.com/in/rasha/'

---